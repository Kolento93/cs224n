...
class SequencePredictor(Model):
    ...
    def add_loss_op(self, preds):
        ...
        #print(tf.shape(y),tf.shape(preds))
    ...
    def add_training_op(self, loss):
        ...
        var_grad = optimizer.compute_gradients(loss)
        grad = [x[0] for x in var_grad]
        var = [x[1] for x in var_grad]
        if self.config.clip_gradients: 
            grad,_ = tf.clip_by_global_norm(grad,self.config.max_grad_norm)

        self.grad_norm = tf.global_norm(grad)
        var_grad = [(grad[i],var[i]) for i in range(len(grad))]
        train_op = optimizer.apply_gradients(var_grad)
