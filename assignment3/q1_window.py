...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(1,len(sentence) - 1):
            tmp = sentence[max(0,i - window_size):i + window_size + 1]
            tmp = reduce(lambda x,y : x + y,tmp)
            windowed_data.append((tmp,labels[i - 1]))
    ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('NER') as scope:
            W = tf.get_variable(name = 'W',shape = [self.config.n_window_features * self.config.embed_size,\
                                                    self.config.hidden_size],dtype = tf.float32,initializer = \
                                                    tf.contrib.layers.xavier_initializer())
            b1 = tf.get_variable(name = 'b1',shape = [1,self.config.hidden_size],initializer = tf.contrib.layers.xavier_initializer())
            b2 = tf.get_variable(name = 'b2',shape =[1,self.config.n_classes],initializer = tf.contrib.layers.xavier_initializer())
